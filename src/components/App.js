...
class App extends React.Component {
    ...
    handleCardLike = (card) => {
        const isLiked = card.likes.some(cardLiker => cardLiker._id === this.state.currentUser._id)


        api.changeLikeStatus(card._id, isLiked)
        .then(res => {
            this.setState({ cards: this.state.cards.map(i => i._id === card._id ? res : i) })
    ...
    handleCardDeleteSubmit = () => {
        ...
        api.deleteCard(this.state.deletingCard._id)
        .then(res => {
            this.setState({ cards: this.state.cards.filter(i => i._id !== this.state.deletingCard._id) })
            this.closeAllPopups()
        })
        .catch(rej => console.error((`Error: ${rej.status}`)))
    ...
    handleUpdateUserSubmit = ({profileName, profileJob}) => {
        this.setState({isLoading: true})
        api.setUserInfo({name: profileName, about: profileJob})
        .then(newUserInfo => {
